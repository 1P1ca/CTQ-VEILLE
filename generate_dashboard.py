...
<div class="topbar">
  ...
  <nav style="display:flex;align-items:center;gap:4px;margin:0 24px;">
    <a href="index.html" style="display:flex;align-items:center;gap:6px;background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);color:#f0f0f0;text-decoration:none;font-size:12px;font-weight:700;padding:7px 16px;border-radius:6px;">📋 Tableau de bord</a>
    <a href="analytics.html" style="display:flex;align-items:center;gap:6px;background:transparent;border:1px solid rgba(255,255,255,0.1);color:#9ca3af;text-decoration:none;font-size:12px;font-weight:600;padding:7px 16px;border-radius:6px;transition:all 0.2s;" onmouseover="this.style.background='rgba(232,75,28,0.15)';this.style.color='#ff7f5c';this.style.borderColor='rgba(232,75,28,0.4)'" onmouseout="this.style.background='transparent';this.style.color='#9ca3af';this.style.borderColor='rgba(255,255,255,0.1)'">📊 Analytique des permis</a>
  </nav>
...
async function loadFeed(feed, label, tabEl) {{
  ...
  try {{
    ...
    items.forEach(item => {{
      ...
      const clean = desc.replace(/<br[^>]*>/gi, '|').replace(/<[^>]+>/g, '').trim();
      const lines = clean.split('|').map(l => l.trim()).filter(l => l.length > 2);
      ...
      lines.forEach(l => {{
        ...
        if (l.includes("introduction")) dateIntro = l.replace("Date d'introduction :", "").trim();
        if (l.match(/^[A-Z][A-Z .]+INC[.]|LTEE|ENRG[.]|TRANSPORT|AUTOBUS|GROUPE/)) demandeur = l;
      ...
      if (titleLow.includes("maintien") || titleLow.includes("acquisition") || titleLow.includes("transfert")) {{ mvLabel = "Transfert"; mvColor = "#8B5CF6"; }}
      ...
      else if (titleLow.includes("revocation") || titleLow.includes("révocation")) {{ mvLabel = "Révocation"; mvColor = "#EF4444"; }}
